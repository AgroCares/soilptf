...
#' @import data.table
#' 
#' @references Adadu et al. (1997) A COMPARISON OF THE CONTRIBUTIONS OF CLAY, SILT, AND ORGANIC MATTER TO THE EFFECTIVE CEC OF SOILS OF SUBSAHARAN AFRICA.
#'
#' @export
...
sptf_cec31 <- function(A_SOM_LOI, A_CLAY_MI,B_SOILCLASS_USDA = NA) {
  ...
  dt[B_SOILCLASS_USDA == 'alfisol', value := 9 + 0.19 * A_CLAY_MI + 0.88 * A_SOM_LOI]
  dt[B_SOILCLASS_USDA == 'entisol', value := 0.9 + 0.33 * A_CLAY_MI + 3.01 * A_SOM_LOI]
  dt[B_SOILCLASS_USDA == 'inceptisol', value := 3.2 + 0.29 * A_CLAY_MI + 2.44 * A_SOM_LOI]
  dt[B_SOILCLASS_USDA == 'ultisol', value := 1.1 + 0.40 * A_CLAY_MI + 2.12 * A_SOM_LOI]
...
sptf_cec32 <- function(A_SOM_LOI, A_CLAY_MI, A_SILT_MI, A_PH_CC) {
  ...
  # function for CEC at pH 7 (NH4Ac)
...
#' @import data.table
#' 
#' @references Manrique et al. (1991). Predicting cationexchange capacity from soil physical and chemical properties. Cited in Liao & Zhu (2015).
#'
#' @export
...
#' @import data.table
#' 
#' @references Franzmeier et al. (1990). Relation of cation exchange capacity to clay and organic carbon contents of Indiana soils. Cited in Liao & Zhu (2015).
#'
#' @export
sptf_cec37 <- function(A_C_OF, A_CLAY_MI) {
  ...
  # function for CEC(n = , R2 = )
  dt[, value := 0.52 + 0.59 * A_CLAY_MI + 3.58 * A_C_OF]
...
#' @import data.table
#' 
#' @references Arnaud & Sephton (1972).  Contribution of clay and organic matter to cation-exchange capacity of chemozemic soils. Cited in Liao & Zhu (2015).
#'
#' @export
sptf_cec38 <- function(A_C_OF, A_CLAY_MI) {
  ...
  # function for CEC(n = , R2 = )
  dt[, value := 0.57 * A_CLAY_MI + 3.84 * A_C_OF]
...
#' @import data.table
#' 
#' @references Miller (1970). Inter-regional predictability of cation-exchange capacity by multiple regression. Cited in Liao & Zhu (2015).
#'
#' @export
sptf_cec40 <- function(A_SOM_LOI, A_CLAY_MI) {
  ...
  # function for CEC(n = , R2 = )
  dt[, value := -1.07 + 0.62 * A_CLAY_MI + 1.78 * A_SOM_LOI]
...
sptf_cec41 <- function(A_SOM_LOI, A_CLAY_MI, A_SILT_MI, A_PH_CC) {
  ...
  dt[, v2 := 5.6719 * exp(0.2109 * A_SOM_LOI)]
